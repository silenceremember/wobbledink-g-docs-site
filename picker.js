(() => {
  ...
  const token=value('token'),state=value('state'),redirectRaw=value('redirect');
  ...
  if(!validRedirect||!/^[a-f0-9]{64}$/u.test(state)||token.length<20||token.length>4096||/\s/u.test(token)){fail('The extension sent an invalid Picker request. Close this window and try again.');return;}
